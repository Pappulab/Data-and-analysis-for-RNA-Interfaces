...
if __name__=="__main__":
    ...
    parser.add_argument("-chains_index", type = int, nargs='+', help="chains index list; used if dist_chains > 1")
    ...
    parser.add_argument("-positions", type = int, nargs='+', help="first and last positions along chain to calculate radial density")
        ...
        for box in box_list:
            ...
            for pre_box in pre_box_list:
                ...
                for chain_index, chains in enumerate(chains_list):
                    ...
                    file_name_nu_dist = ana_path + name + '_' + str(temp) + '_' + str(box) + '_' + str(pre_box) + '_' + str(cur_chain_index) + '_Nu_Distribution.dat'
                    file_name_rg_full = ana_path + name + '_' + str(temp) + '_' + str(box) + '_' + str(pre_box) + '_' + str(cur_chain_index) + '_Rg.dat'
                    file_name_rg_dist = ana_path + name + '_' + str(temp) + '_' + str(box) + '_' + str(pre_box) + '_' + str(cur_chain_index) + '_Rg_Distribution.dat'
                    ...
                    file_name_COM_pos = ana_path + name + '_' + str(temp) + '_' + str(box) + '_' + str(pre_box) + '_' + str(cur_chain_index) + '_COM_pos_Distribution.dat'
                    ...
                    file_name_cosine_dist = ana_path + name + '_' + str(temp) + '_' + str(box) + '_' + str(pre_box) + '_' + str(cur_chain_index) + '_Cosine_Distribution.dat'
                    ...
                    if trj_check == True:
                        ...
                        if nu_check == 1:
                            ...
                            with open(file_name_nu_dist, 'w+') as fp:
                                fp.write('#Chain_Type\tRadial_Bin\tNu\n')
                            with open(file_name_rg_full, 'w+') as fp:
                                fp.write('#Chain_Type\tRg_Dense\tRg_Dilute\n')
                            with open(file_name_rg_dist, 'w+') as fp:
                                fp.write('#Chain_Type\tClus_Check\tRadial_Bin\tRg_Squared\n')
                        ...
                        if com_dist_check == 1:
                            ...
                            with open(file_name_COM_pos, 'w+') as fp:
                                fp.write('#COM_Bin\tbeads_per_bin\n')
                                fp.write('#First column is beads within defined range\n')
                                fp.write('#Second column is beads outside the range\n')
                                fp.write('#The range is %i to %i\n' % (pos_list[0], pos_list[1]))
                            ...
                            with open(file_name_COM_solvation, 'w+') as fp:
                                ...
                                fp.write('#Order is chain type\n')
                            with open(file_name_cosine_dist, 'w+') as fp:
                                fp.write('#COM_Bin\tcosine_squared\n')
                                fp.write('#Distribution comes from 1% of samples chosen at random\n')
                        ...
                        for run_it in range(runs_per_condition):
                            ...
                            nu_dist_array = np.array([file_name_nu_dist.encode('utf-8')])
                            nu_dist_array_view = nu_dist_array.view('uint8').reshape(nu_dist_array.size, nu_dist_array.dtype.itemsize)
                            ...
                            rg_dist_array = np.array([file_name_rg_dist.encode('utf-8')])
                            rg_dist_array_view = rg_dist_array.view('uint8').reshape(rg_dist_array.size, rg_dist_array.dtype.itemsize)
                            ...
                            COM_pos_dist_array = np.array([file_name_COM_pos.encode('utf-8')])
                            COM_pos_dist_array_view = COM_pos_dist_array.view('uint8').reshape(COM_pos_dist_array.size, COM_pos_dist_array.dtype.itemsize)
                            ...
                            cosine_dist_full_array = np.array([file_name_cosine_dist.encode('utf-8')])
                            cosine_dist_full_array_view = cosine_dist_full_array.view('uint8').reshape(cosine_dist_full_array.size, cosine_dist_full_array.dtype.itemsize)
                            ...
                            Pos_array = np.array(pos_list, dtype='int')
                            ...
                            traj_analyzer_multi_new.Frame_Loader(array_view.shape[0], binary_check, array_view, array_view.shape[1],
                                                                 ...
                                                                 nu_array_view, nu_array_view.shape[1],
                                                                 nu_dist_array_view, nu_dist_array_view.shape[1],
                                                                 rg_full_array_view, rg_full_array_view.shape[1],
                                                                 rg_dist_array_view, rg_dist_array_view.shape[1],
                                                                 re_dist_array_view, re_dist_array_view.shape[1],
                                                                 gtensor_dist_array_view, gtensor_dist_array_view.shape[1],
                                                                 fractal_ana_dist_array_view, fractal_ana_dist_array_view.shape[1],
                                                                 rg_COM_dist_array_view, rg_COM_dist_array_view.shape[1],
                                                                 COM_bin_count_dist_array_view, COM_bin_count_dist_array_view.shape[1],
                                                                 COM_bead_dist_array_view, COM_bead_dist_array_view.shape[1],
                                                                 COM_pos_dist_array_view, COM_pos_dist_array_view.shape[1],
                                                                 COM_chain_dist_array_view, COM_chain_dist_array_view.shape[1],
                                                                 COM_sticker_dist_array_view, COM_sticker_dist_array_view.shape[1],
                                                                 COM_Re_dist_array_view, COM_Re_dist_array_view.shape[1],
                                                                 COM_angle_dist_array_view, COM_angle_dist_array_view.shape[1],
                                                                 COM_dot_prod_dist_array_view, COM_dot_prod_dist_array_view.shape[1],
                                                                 COM_solvation_dist_array_view, COM_solvation_dist_array_view.shape[1],
                                                                 RDF_dist_array_view, RDF_dist_array_view.shape[1],
                                                                 mol_COM_dist_array_view, mol_COM_dist_array_view.shape[1],
                                                                 mol_COM_dist_array_view_2, mol_COM_dist_array_view_2.shape[1],
                                                                 mol_COM_dist_array_view_3, mol_COM_dist_array_view_3.shape[1],
                                                                 displacement_dist_array_view, displacement_dist_array_view.shape[1],
                                                                 sticker_cluster_dist_array_view, sticker_cluster_dist_array_view.shape[1],
                                                                 crosslink_dist_array_view, crosslink_dist_array_view.shape[1],
                                                                 solvation_dist_array_view, solvation_dist_array_view.shape[1],
                                                                 COM_gtensor_dist_array_view, COM_gtensor_dist_array_view.shape[1],
                                                                 connectedness_dist_array_view, connectedness_dist_array_view.shape[1],
                                                                 connectedness_dist_binned_array_view, connectedness_dist_binned_array_view.shape[1],
                                                                 connectedness_dist_full_array_view, connectedness_dist_full_array_view.shape[1],
                                                                 sticker_lifetime_dist_full_array_view, sticker_lifetime_dist_full_array_view.shape[1],
                                                                 cosine_dist_full_array_view, cosine_dist_full_array_view.shape[1],
                                                                 concave_hull_dist_full_array_view, concave_hull_dist_full_array_view.shape[1],
                                                                 cluster_dist_array_view, cluster_dist_array_view.shape[1],
                                                                 time_start, time_inc, time_end, time_cutoff,
                                                                 int(nu_check), int(re_dist_check), int(gtensor_dist_check),
                                                                 ...
                                                                 Sticker_array, Sticker_array_length, Pos_array,
                            ...
                            if nu_check == 1:
                                with open(file_name_nu, 'a') as fp:
                                    fp.write('#NewRun\n')
                                with open(file_name_nu_dist, 'a') as fp:
                                    fp.write('#NewRun\n')
                            ...
                            if com_dist_check == 1:
                                ...
                                with open(file_name_COM_pos, 'a') as fp:
                                    fp.write('#NewRun\n')
                                ...
                                with open(file_name_cosine_dist, 'a') as fp:
                                    fp.write('#NewRun\n')
