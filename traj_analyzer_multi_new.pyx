...
cdef:
    int file_num[1]
    int nu_file_size[1]
    int nu_dist_file_size[1]
    int rg_full_file_size[1]
    int rg_dist_file_size[1]
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef COM_Pos_Writer(char* COM_pos_file, long *COM_pos_array):
     
    cdef:
        int i, j
        long double cur_bin_pos
        FILE* f_COM_pos
     
    f_COM_pos = fopen(COM_pos_file[:COM_pos_dist_file_size[0]], "a")
    for i in range(4 * box_size[0]):
        fprintf(f_COM_pos, "%d", i)
        for j in range(2):
            cur_bin_pos = <long double> COM_pos_array[2 * i + j] / <long double> count_analysis[0]
            fprintf(f_COM_pos, "\t%LE", cur_bin_pos)
        fprintf(f_COM_pos, "\n")
    fclose(f_COM_pos)
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef Cosine_Distribution_Writer(char* cosine_dist_file, long double *cosine_array,
                                int *bin_array):
     
    cdef:
        int i, j
        long double rand_val
        FILE* f_COM_angle_dist
     
    f_COM_angle_dist = fopen(cosine_dist_file[:cosine_dist_file_size[0]], "a")
    for i in range(num_beads[0]):
        if cosine_array[i] < 1:
            rand_val = <long double> (rand() / (RAND_MAX + 1.0))
            if rand_val < 0.01:
                fprintf(f_COM_angle_dist, "%d\t%LE\n", bin_array[i], cosine_array[i])
    fclose(f_COM_angle_dist)
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef Nu_Distribution_Writer(char* nu_dist_file, long double *temp_nu_array,
                            int *bin_array, int *topo_array):
    ...
    f_nu_dist = fopen(nu_dist_file[:nu_dist_file_size[0]], "a")
    for i in range(num_beads[0]):
        chain_id = topo_array[TOPO_ARRAY_MAX * i + TOPO_ARRAY_CHAIN]
        rand_val = <long double> (rand() / (RAND_MAX + 1.0))
        if rand_val < 0.01:
            fprintf(f_nu_dist, "%d\t%d\t%LE\n", chain_id, bin_array[i], temp_nu_array[chain_id])
    fclose(f_nu_dist)
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef Rg_Distribution_Writer(char* rg_dist_file, long double *rg_array,
                            int *bin_array, int *topo_array, int *clus_checklist):
     
    cdef:
        int i
        int chain_id, clus_check
        long double rand_val
        FILE* f_rg_dist
     
    f_rg_dist = fopen(rg_dist_file[:rg_dist_file_size[0]], "a")
    for i in range(num_beads[0]):
        chain_id = topo_array[TOPO_ARRAY_MAX * i + TOPO_ARRAY_CHAIN]
        clus_check = clus_checklist[chain_id]
        rand_val = <long double> (rand() / (RAND_MAX + 1.0))
        if rand_val < 0.01:
            fprintf(f_rg_dist, "%d\t%d\t%d\t%LE\n", chain_id, clus_check, bin_array[i], rg_array[chain_id])
    fclose(f_rg_dist)
...
cdef COM_Distance_Analysis(int *frame_array, int *topo_array, int *lattice_array, int *system_COM,
                           ...
                           long *COM_bead_array, long *COM_pos_array, long *COM_chain_array, long *COM_count_array,
                           ...
                           long double *cosine_array, int *sticker_array, int *pos_array,
                           const int *chain_id_to_distinct_chain_array,
                           const int *chain_id_to_first_bead_array,
    ...
    cdef:
        int i, j, k, m, n
        int b_initial, b_final, cur_dist, cur_pos
    ...
    for i in range(num_beads[0]):
        ...
        chain_1_length = 100
        b_initial = chain_id_to_first_bead_array[chain_1]
        cur_pos = i - b_initial
        if cur_pos >= pos_array[0] and cur_pos <= pos_array[1]:
            pos_bin = 0
        else:
            pos_bin = 1
        ...
        COM_pos_array[2 * bin_1 + pos_bin] += 1
...
cdef Cosine_Analysis(int *frame_array, int *topo_array,
    ...
    bin_array = <int *> malloc(2 * num_beads[0] * sizeof(int))
    ...
    Cosine_Distribution_Writer(cosine_dist_file, cosine_array, bin_array)
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef Scaling_Analysis(int *frame_array, int *topo_array, long double *scaling_array_clus,
                      ...
                      const int *scaling_length_fb_array, const int *scaling_length_array,
                      const int *system_COM, char *nu_dist_file):
    ...
    cdef:
        ...
        int *clus_checklist
        int *bin_array
        long double *temp_scaling_array
        long double *temp_nu_array
    
    clus_checklist = <int *> calloc(total_num_chains[0], sizeof(int))
    bin_array = <int *> malloc(num_beads[0] * sizeof(int))
    temp_scaling_array = <long double *> calloc(total_scaling_length[0], sizeof(long double))
    temp_nu_array = <long double *> calloc(total_num_chains[0], sizeof(long double))
        ...
        if clus_checklist[i] == 1:
            for j in range(fb, lb-1):
                for k in range(j+1, lb):
                    cur_dist = Dist_Bead_Bead_Squared_No_PBC(frame_array, j, k)
                    scaling_array_clus[scaling_array_fb + counter] += cur_dist
                    temp_scaling_array[scaling_array_fb + counter] = cur_dist
                    counter += 1
        else:
            for j in range(fb, lb-1):
                for k in range(j+1, lb):
                    cur_dist = Dist_Bead_Bead_Squared(frame_array, j, k)
                    scaling_array_solo[scaling_array_fb + counter] += cur_dist
                    temp_scaling_array[scaling_array_fb + counter] = cur_dist
                    counter += 1
        counter = 0
        
    #===========================================================================
    #     Nu_Calculator(temp_scaling_array, chain_id_to_distinct_chain_array[i],
    #                   scaling_length_fb_array, scaling_length_array,
    #                   chain_length_array, 0, 25)
    #     temp_nu_array[i] = nu_mean[0]
    # 
    # Nu_Distribution_Writer(nu_dist_file, temp_nu_array, bin_array, topo_array)
    #===========================================================================

    free(clus_checklist)
    free(bin_array)
    free(temp_scaling_array)
    free(temp_nu_array)

@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cdef Rg_Full_Analysis(int *frame_array, int *topo_array, long double *rg_array,
                      long double *rg_full_array, long *rg_full_count_array,
                      const int *cluster_list_largest, const int clus_size,
                      const int *chain_id_to_distinct_chain_array,
                      const int *system_COM, char *rg_dist_file):
    ...
    Rg_Distribution_Writer(rg_dist_file, rg_array, bin_array, topo_array, clus_checklist)
...
cdef Perform_Analysis(int *frame_array, int *topo_array, int *lattice_array,
                      ...
                      int *cluster_size_dilute_array, int *cluster_size_dense_array,
                      int *sticker_array, int *pos_array, int *bead_sticker_index_array,
                      ...
                      char *nu_dist_file, char *rg_dist_file, char *re_file_clus, char *re_file_solo,
                      ...
                      char *sticker_lifetime_file, char *cosine_dist_file):
    ...
    if fractal_ana_dist_check[0] == 1 or com_dist_check[0] == 1 or mol_COM_dist_check[0] == 1 or com_gtensor_dist_check[0] == 1 or connectedness_dist_check[0] == 1 or concave_hull_dist_check[0] == 1 or nu_check[0] == 1:
    ...
    if nu_check[0] == 1:
        Scaling_Analysis(frame_array, topo_array, scaling_array_clus, scaling_array_solo,
                         cluster_list_largest, clus_size_largest,
                         chain_id_to_distinct_chain_array,
                         chain_id_to_first_bead_array, chain_length_array,
                         scaling_length_fb_array, scaling_length_array, system_COM,
                         nu_dist_file)
        Rg_Full_Analysis(frame_array, topo_array, rg_array, rg_full_array,
                         rg_full_count_array, cluster_list_largest, clus_size_largest,
                         chain_id_to_distinct_chain_array, system_COM, rg_dist_file)
    ...
    if com_dist_check[0] == 1:
        ...
        COM_Distance_Analysis(frame_array, topo_array, lattice_array, system_COM, COM_Rg_array,
                              COM_bin_count_array, COM_bead_array, COM_pos_array, COM_chain_array,
                              ...
                              pos_array, chain_id_to_distinct_chain_array, chain_id_to_first_bead_array,
...
cpdef Frame_Loader(const int num_files, const int binary_check, char[:, ::1] trj_file_list,
                   ...
                   const int nu_file_size_1, char[:, ::1] nu_dist_file_list,
                   const int nu_file_size_2, char[:, ::1] rg_full_file_list,
                   const int rg_file_size_1, char[:, ::1] rg_dist_file_list,
                   const int rg_file_size_2, char[:, ::1] re_dist_file_list,
                   ...
                   const int COM_bead_file_size, char[:, ::1] COM_pos_file_list,
                   const int COM_pos_file_size, char[:, ::1] COM_chain_file_list,
                   ...
                   const int sticker_lifetime_file_size, char[:, ::1] cosine_dist_file_list,
                   const int cosine_file_size, char[:, ::1] concave_hull_file_list,
                   ...
                   const int sticker_array_entry_length, long[:] pos_array_entry,
                   const int fractal_ana_samples_entry,
    ...
    nu_file_size[0] = nu_file_size_1
    nu_file = &nu_file_list[0, 0]

    nu_dist_file_size[0] = nu_file_size_2
    nu_dist_file = &nu_dist_file_list[0, 0]

    rg_full_file_size[0] = rg_file_size_1
    rg_full_file = &rg_full_file_list[0, 0]

    rg_dist_file_size[0] = rg_file_size_2
    rg_dist_file = &rg_dist_file_list[0, 0]
    ...
    COM_pos_dist_file_size[0] = COM_pos_file_size
    COM_pos_file = &COM_pos_file_list[0, 0]
    ...
    cosine_dist_file_size[0] = cosine_file_size
    cosine_dist_file = &cosine_dist_file_list[0, 0]
    ...
    pos_array = <int *> calloc(2, sizeof(int))
    for i in range(2):
        pos_array[i] = pos_array_entry[i]
    ...
    if com_dist_check[0] == 1:
        ...
        COM_pos_array = <long *> calloc(4 * box_size[0] * 2, sizeof(long))
        ...
        if binary_check == 1:
            ...
            while cur_frame < tot_frames:
                ...
                Perform_Analysis(frame_array, topo_array, lattice_array, scaling_array_clus,
                                 ...
                                 COM_bead_array, COM_pos_array, COM_chain_array, COM_count_array,
                                 ...
                                 cluster_size_dense_array, sticker_array, pos_array,
                                 bead_sticker_index_array, crosslink_ana_chain_array,
                                 crosslink_ana_full_array, crosslink_ana_full_count_array,
                                 concave_hull_array, concave_hull_com_array,
                                 num_chains_array, chain_id_to_distinct_chain_array,
                                 chain_id_to_first_bead_array, chain_length_array,
                                 count_clus_array, count_solo_array, nu_dist_file, rg_dist_file,
                                 ...
                                 sticker_lifetime_file, cosine_dist_file)
                cur_frame += 1
    ...
    if com_dist_check[0] == 1:
        ...
        COM_Pos_Writer(COM_pos_file, COM_pos_array)
